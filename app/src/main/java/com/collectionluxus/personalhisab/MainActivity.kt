...
data class Entry(
    ...
    val party: String, val item: String, val amount: Double, val received: Double,
    val pending: Double, val account: String, val fromAccount: String,
    val toAccount: String, val notes: String
)

class HisabDb(context: android.content.Context) :
    SQLiteOpenHelper(context, "personal_hisab.db", null, 1) {
    override fun onCreate(db: SQLiteDatabase) {
        db.execSQL("""
            CREATE TABLE entries(
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                date TEXT NOT NULL, type TEXT NOT NULL, title TEXT,
                party TEXT, item TEXT, amount REAL NOT NULL DEFAULT 0,
                received REAL NOT NULL DEFAULT 0, pending REAL NOT NULL DEFAULT 0,
                account TEXT, from_account TEXT, to_account TEXT, notes TEXT
            )
        """.trimIndent())
    }
    ...
    fun insert(e: Entry) {
        writableDatabase.insert("entries", null, ContentValues().apply {
            ...
            put("party", e.party); put("item", e.item); put("amount", e.amount)
            put("received", e.received); put("pending", e.pending)
...
fun PersonalHisabApp(db: HisabDb) {
    ...
    MaterialTheme {
        ...
        if (showAdd) AddEntryDialog(
            ...
            onSave = { db.insert(it); refresh(); showAdd = false }
        )
    }
}
...
fun AccountCard(name: String, entries: List<Entry>, account: String) {
    Card(Modifier.fillMaxWidth()) {
        ListItem(
            ...
            trailingContent = { Text(money(accountBalance(entries, account)), style = MaterialTheme.typography.titleMedium) }
        )
    }
}
    ...
    AlertDialog(
        ...
        text = {
            Column(Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(7.dp)) {
                ...
                if (type != "Transfer") {
                    ...
                    Text("To")
                    FlowRow(horizontalArrangement = Arrangement.spacedBy(5.dp)) {
                        accounts.forEach { a -> FilterChip(selected = toAccount == a, onClick = { toAccount = a }, label = { Text(a) }) }
                    }
                }
        ...
        confirmButton = {
            Button(onClick = {
                ...
                onSave(Entry(
                    0, SimpleDateFormat("yyyy-MM-dd", Locale.US).format(Date()), type,
                    title.ifBlank { type }, party, item, total, rec,
                    if (type == "Sale") total - rec else 0.0,
                    account, if (type == "Transfer") account else "",
                    if (type == "Transfer") toAccount else "", notes
                ))
            }) { Text("Save") }
        },
